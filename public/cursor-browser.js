...
class WebSocketManager {
    constructor() {
        this.ws = null;
        this.url = 'ws://localhost:3000'; // 修复：移除 /ws
        this.isConnecting = false;
        this.retryCount = 0;
        this.maxRetries = 5;
        this.retryDelay = 2000;
...
class CursorSync {
    ...
    async syncContent() {
        try {
            ...
            const response = await fetch(`${this.serverUrl}/api/content`, { // 修复：使用 /api/content
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
...
window.debugCursorSync = () => {
    ...
    // 测试内容获取
    const content = sync.getContentPayload();
    if (content) {
        console.log('✅ 当前内容：', content.data.html.length, '字符');
    } else {
        console.log('❌ 内容获取失败');
    }
