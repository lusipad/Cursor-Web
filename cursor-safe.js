// 🚀 Cursor 安全版同步脚本 - 避免TrustedHTML问题
console.log('🚀 Cursor 安全版同步脚本开始运行...');

class SafeCursorSync {
    constructor() {
        this.serverUrl = 'http://localhost:3000';
        this.lastContent = '';
        this.syncInterval = null;

        console.log('🔧 初始化安全版同步系统...');
        this.init();
    }
    ...
    async init() {
        try {
            ...
            this.showMessage('✅ 安全版同步已启动', '#4CAF50');
            console.log('🎉 安全版同步系统初始化完成');
        } catch (error) {
    ...
    getContent() {
        try {
            ...
            console.log('📋 安全抓取内容成功:', result.html.length, '字符');
            console.log('🎯 聊天容器数量:', chatContainers.length);
            return result;

        } catch (error) {
            console.error('❌ 安全抓取内容失败:', error);
            return null;
        }
    }
    ...
    textToHtml(text) {
        // 创建一个安全的HTML版本
        return `
            <div style="font-family: monospace; white-space: pre-wrap; padding: 20px; background: #1a1a1a; color: #ffffff;">
...
// 检查是否已经运行
if (window.safeCursorSync) {
    console.log('⚠️ 安全版脚本已在运行');
    alert('安全版脚本已在运行中！');
} else {
    // 启动脚本
    console.log('🚀 启动安全版同步脚本...');
    window.safeCursorSync = new SafeCursorSync();

    // 提供手动同步方法
    window.manualSync = () => {
        if (window.safeCursorSync) {
            window.safeCursorSync.manualSync();
        }
    };
    ...
    window.stopSync = () => {
        if (window.safeCursorSync) {
            window.safeCursorSync.stop();
            window.safeCursorSync = null;
        }
    };
}

console.log('✅ 安全版同步脚本加载完成');
console.log('💡 使用方法：');
